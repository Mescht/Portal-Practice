...
# add element to gui
execute if score disabled gui matches 0 run data modify storage portal:gui settings append value {id:"minecraft:glowstone_dust",Count:1b,tag:{display:{Lore:['{"text":"Enabled","color":"green","italic":"false"}']}}}
execute if score disabled gui matches 1 run data modify storage portal:gui settings append value {id:"minecraft:gunpowder",Count:1b,tag:{display:{Lore:['{"text":"Disabled","color":"red","italic":"false"}']}}}
