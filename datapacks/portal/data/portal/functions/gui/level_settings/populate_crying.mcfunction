...
execute if score disabled filter matches 1 run data modify storage portal:gui settings[{tag:{index:25b}}].tag.display.Lore set value ['{"text":"Enabled","color":"green","italic":"false"}']
execute if score disabled filter matches 0 run data modify storage portal:gui settings[{tag:{index:25b}}].tag.display.Lore set value ['{"text":"Disabled","color":"red","italic":"false"}']
